"""Image convert resource: client.images (canonical /api/v1/convert/image/)."""
...
from .._paths import IMAGE_PREFIX
...
class Images:
    ...
    def formats(self) -> dict[str, Any]:
        return as_dict(self._client._http.get(f"{IMAGE_PREFIX}/formats/"))

    def operations(self) -> dict[str, Any]:
        return as_dict(self._client._http.get(f"{IMAGE_PREFIX}/operations/"))

    def history(self, **params: Any) -> dict[str, Any]:
        return as_dict(
            self._client._http.get(f"{IMAGE_PREFIX}/history/", params=params or None)
        )
            ...
            with path.open("rb") as fh:
                return as_dict(
                    self._client._http.post(
                        f"{IMAGE_PREFIX}/convert/",
                        data=form,
                        files={"file": (path.name, fh)},
                    )
                )
        return as_dict(
            self._client._http.post(f"{IMAGE_PREFIX}/convert/", data=form)
        )

    def get(self, conversion_id: str) -> dict[str, Any]:
        return as_dict(
            self._client._http.get(f"{IMAGE_PREFIX}/conversion/{conversion_id}/")
        )
